...
#清空各变量的Unknown值
table(seer$Marital_status)#婚姻
seer <- subset(seer,seer$Marital_status!='Unknown')

table(seer$Race)#种族
seer <- subset(seer,seer$Race!='Unknown')

table (seer$Tumor_stage)#肿瘤情况
seer <- subset(seer,seer$Tumor_stage!='Unknown/unstaged')

table(seer$RegionalLNSurgeryExtent)#区域淋巴结手术
seer <- subset(seer,seer$RegionalLNSurgeryExtent!='Unknown or not applicable')

table(seer$Tumor_size)#肿瘤尺寸
seer <- subset(seer,seer$Tumor_size!=999)#999=Unknow

table(seer$Differentiation)#肿瘤分级
seer <- subset(seer,seer$Differentiation!='Unknown')

table(seer$PrimarySiteSurgeryType)#原发部位手术
seer <- subset(seer,seer$PrimarySiteSurgeryType!=99)#99=Unknow
